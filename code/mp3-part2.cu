...
bool allocate_device_storage(unsigned int *&d_input, unsigned int *&d_compacted_result, size_t n)
{
  ...
  cudaMalloc((void **) &d_compacted_result, n*sizeof(unsigned int));
  if(d_input == 0 || d_compacted_result == 0)
    return false;
  else
    return true;
...
// size_t num_elements = 1<<16;
  ...
  // copy input to GPU
  start_timer(&timer);
  cudaMemcpy(d_input, &h_input[0], num_elements * sizeof(unsigned int), cudaMemcpyHostToDevice);
  //cudaMemcpy(d_input, h_input, num_elements * sizeof(unsigned int), cudaMemcpyHostToDevice);
