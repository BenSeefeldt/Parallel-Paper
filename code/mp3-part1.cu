...
//Allocates space on CUDA device. Returns success.
...
//Deallocates the space allocated by above method.
...
int main(void)
{
  ...
  for(int round = 1; round < num_subsequent_rounds + 1; ++round)
  {
    ...
    start_timer(&timer);
    naively_filtered_black_scholes_kernel<<< nBlocks, blockSize>>>(d_stock_price, d_option_strike, d_option_years,
                                                                   d_first_round_call_result, d_first_round_put_result,
                                                                   d_subsequent_round_call_result, d_subsequent_round_put_result,
                                                                   min_call_threshold, min_put_threshold, riskless_rate,
                                                                   volatility, num_options);
    char message[256];
